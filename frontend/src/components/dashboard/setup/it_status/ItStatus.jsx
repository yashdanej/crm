...
import { getIt_Status, get_Group } from '../../../../store/slices/SetupSlices';
import ItStatusTable from './ItStatusTable';
...
const ItStatus = () => {
    ...
    const handleStatusSubmit = (e) => {
        ...
        if (editDesignationId) {
            ...
            api(`/util/it_status/${editDesignationId}`, "patch", { name: name }, false, true)
            .then((res) => {
                setName("");
            ...
            // Add new agent
            api("/util/it_status", "post", { name: name }, false, true)
            .then((res) => {
                setName("");
                fetchDesignationsData();
            })
    ...
    const handleEdit = (id) => {
        api(`/util/it_status/${id}`, "get", false, false, true)
        .then((res) => {
...
useEffect(() => {
    ...
    return (
        <div className='m-6'>
            <h1 className='text-3xl font-bold bg-gray-900 text-white p-5'>It Status Master</h1>
            <div className='xl:flex gap-10 my-6'>
                ...
                <div className='my-6'>
                    <ItStatusTable handleEdit={handleEdit} />
                </div>
            </div>
        </div>
    )
